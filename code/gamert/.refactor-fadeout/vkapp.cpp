...
void VKApplication::init()
{
	...
	_create_swapchain();
	rmgr.set_vulkan_swapchain_image_format(_vkscimgfmt);
	rmgr.set_vulkan_swapchain_extent(_vkscext);

	_create_image_views();
	rmgr.create_renderpasses();
	rmgr.create_pipelines();
	
	_create_frame_buffers();
	
	_create_cmd_pool();
	rmgr.set_vulkan_command_pool(_vkcmdpool);
...
void VKApplication::_clean_up_swapchain()
{
	...
	RenderMgr::get_instance().destroy_renderpasses();
...
void VKApplication::_drawframe()
{
	...
	RenderMgr::get_instance().render_frame(_vksc_framebuffers[_cur_frame_idx]);
	std::vector<VkCommandBuffer> cmds = RenderMgr::get_instance().get_vulkan_commands();
	...
	submit_info.commandBufferCount = (uint32_t)cmds.size();
	...
	RenderMgr::get_instance().clear_vulkan_commands();
